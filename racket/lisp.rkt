...
;;;; (a b) -> [a, b)
;; Precondition: a <= b
;; Returns a list of integers representing the mathematical object [a, b); viz.
;; a list of every integer from a inclusive to b exclusive, from least to
;; greatest.
...
;;;; (function list) -> list
;; Given a function and a list, this function returns a new list whose elements
;; are the result of applying the given function to each element of the original
;; list.
...
;;;; (function list) -> (list)
;; Performs a map operation on the given list, then appends every sublist in the
;; list. This function assumes that each element in the list will be a list
...
;; Given two functions which alter an assoc pair in some way, this function
;; returns a new function for use in conjunction with 'reduce' which performs
;; operations using the two given functions on pairs in an assoc list with
;; common keys. This is a generalization of the case where the first functional
;; argument is 'first', and the second functional argument is 'second'.
;;; Example-usage (group-common-test)
;;  ;; > (reduce (group-common first second)
;;  ;;           '()
;;  ;;           '((Indo-European Latin) (Indo-European Greek) (Jam Grape)
;;  ;;             (Jam Plum) (Indo-European Sanskrit)))
;;  ;; '((Indo-European (Sanskrit Greek . Latin)) (Jam (Plum . Grape)))
(define (group-common-test)
  (let ((result (reduce (group-common first second)
                        '()
                        '((Indo-European Latin) (Indo-European Greek)
                          (Jam Grape) (Jam Plum) (Indo-European Sanskrit))))
        (target '((Indo-European (Sanskrit Greek . Latin))
                  (Jam (Plum . Grape)))))
    (if (equal? result target)
        #t
...
;; Given a function which combines two elements of data, an initial data value,
;; and a list of data elements, this function uses the given function to combine
;; all of the elements of the given list,
;; in FIFO order.
...
;; Given a natural number, this function returns a function which will take one
;; input and return a list which contains that input repeated as many times as
...
;;;; (numbers) -> number
;; Given a list of numbers, this function multiplies them all and returns the
;; product.
...
;; Given a natural-number, this function returns the factorial of that number,
;; viz. the product of the numbers from 1 to that number inclusive.
...
;; Given a function which takes an integer as an input, and two integers a & b
;; where a <= b, this function returns a list of the image of that function on
;; the range [a, b), viz. f[a, b).
...
;; This function returns the sum of the sequence resulting from the given
;; function and two integers.
...
;;;; (function list) -> list
;; Given a function which returns either true or false given any input, and a
;; list of elements which can each be given to that function as inputs, this
;; function filters out from that list all elements which the given function
;; maps to false, returning a list of only those elements which the given
;; function maps to true. In terms of sets, the result of this function is the
;; intersection of the original set O with the fiber of true on f
;; viz. O intersect f^-1{true}.
...
;; Given a function which returns either true or false given any input, and a
;; list of inputs, this function returns true if and only if the given function
;; maps every element of the input list to true. That is to say, this function
;; returns true iff f(A) = {true}, where A is the given list.
...
;;;; (function list) -> boolean
;; Given a function which returns either true or false given any input, and a
;; list of inputs, this function returns true if and only if there exists at
;; least one element in the input list which the given function maps to true.
;; That is to say, this function returns true iff true \in f(A), where A is the
;; given list.
...
;;;; (functions) -> function
;; Given a list of functions, this function returns the function representing
;; the composition of all those functions.
...
;;;; (list) -> (list list)
;; Given a list, this function returns a list of two lists, each containing one
;; half of the original list. If the list has an odd number of elements, the
...
;;;; (list) -> list
;; Given a list of numbers, this function returns that list sorted using the
...
;;;; (pair) -> item
;; This function is a safer version of car, which returns the first element of a
;; pair. If the given pair is null, instead of an error being thrown, null is
;; returned.
;;; Example-usage
;;  ;; > (scar '())
;;  ;; '()
...
;;;; (pair) -> item
;; This function is a safer version of cdr, which returns the second element of
;; a pair, generally being another pair itself. If the given pair is null, this
...
;; This function returns the left child of a tree, or null if the given tree is
;; null.
...
;; This function returns the right child of a tree, or null if the given tree is
;; null.
...
;; Given a tree, this function returns a new tree where the left and right
;; children of the given node are reversed, with the same value.
...
;;;; (key val lst) -> lst
;; Given a key, a value, and a list, this function adds the pair (key . value)
;; to the beginning of the given list, which if used to make an entire list,
;; will create an association list compatible with the assoc function.
;; More explicitly:
...
;; Given a key in an association list, and that list, this function returns the
;; given list with all its elements except for the pair with the provided key.
;; Note, that if the given key does not appear in the list, the list is returned
...
;;  ;; > (assoc-remove 'America
;;  ;;                 '((America . American) (Rome . Latin) (Athens . Greek)))
;;  ;; '((Rome . Latin) (Athens . Greek))
(define (assoc-remove-test)
  (let ((result (assoc-remove 'America
                              '((America . American)
                                (Rome . Latin)
                                (Athens . Greek))))
        (target '((Rome . Latin) (Athens . Greek))))
...
;; Given two strings, -1 is returned if the first is less than the second, by
;; comparing contained characters from first to last by ASCII value.
...
;;;; list -> f : element -> list
;; Given a black-list, this function returns a function which will take an input
;; item and return true if the given item is not in the black-list.
